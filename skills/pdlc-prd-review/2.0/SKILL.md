...
description: Независимая проверка PRD. Применяется на соответствующей ноде HG PDLC 2.0.
---
...
## Workflow
Запусти роль pdlc-prd-reviewer на исходном запросе, analysis, реальных ответах и draft PRD. Не проси автора подтвердить свой же текст. Проверь все обязательные секции, согласованность терминов, атомарность BR, проверяемость AC, scope/out-of-scope, бизнес-инварианты, неизвестные и риски. Для каждого INT/HYP/BR/AC нужен checked_id. Особенно ищи потерянные оговорки источников.

## Outputs
run: review.json. subject_digest = `hgpdlc-v2 subject --node ai-prd-review --kind prd`. on_success только при pass; on_rework при исправимом документе; on_questions при недостающем намерении. Роль не редактирует PRD.
