...
description: Независимый audit тестов. Применяется на соответствующей ноде HG PDLC 2.0.
---

# Независимый audit тестов


## Workflow
Запусти pdlc-test-reviewer с frozen test model, AC/SRS, source тестов и границей стенда. Проверь, что asserts наблюдают обещанный результат, ошибки не поглощаются, авторизация настоящая, persistence проверена, mocks не заменили внутренние компоненты, fixtures независимы, tests не skipped и нет hardcoded waits. Уточни all TEST-ID→executable binding. Включи контрпример: какой намеренный дефект должен уронить ключевой E2E? Не изменяй тесты здесь.

## Outputs
run: review.json; subject_digest=subject --kind code; checked_ids все E2E/CHECK. on_rework → triage, а не автоматический rewrite oracle.
