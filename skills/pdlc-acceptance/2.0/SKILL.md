---
name: pdlc-acceptance
description: Приёмка по доказательствам. Применяется на соответствующей ноде HG PDLC 2.0.
---
...
## Workflow
Запусти pdlc-acceptance-auditor. Сопоставь оригинальное намерение, принятое PRD, решения, SRS, код, исполняемые assertions, native JUnit/logs и trace-result. Для каждого AC/TEST нужен свежий результат конкретного run/node/attempt. Проверь source/graph fingerprints и raw artifact hashes; summary «pass» без файла не доказательство. Для выявленной ошибки верни цепочку ancestors и owner-stage. Outcome/business effect не объявляется достигнутым.

## Outputs
run: review.json; subject_digest=subject --kind acceptance, checked_ids все AC/E2E/CHECK. on_success → финальный deterministic gate, а не публикация; on_rework → triage.
