...
description: Audit восстановленной базы. Применяется на соответствующей ноде HG PDLC 2.0.
---

# Audit восстановленной базы


## Workflow
Запусти pdlc-baseline-auditor. Проверь source coverage, реальные repo/path/commit, различие observed/confirmed/inferred, отсутствие ложного исторического intent и test pass. Сравни source inventory с итоговой картой компонентов, контрактов и тестов. Проверь, что записи inferred не normative. Ревью не принимает за владельца продукта неизвестные бизнес-правила.

## Outputs
run: review.json. subject_digest=`hgpdlc-v2 baseline-subject --node ai-baseline-review`. checked_ids содержит все FACT-ID; findings явно называют gaps. on_success → command gate → human acceptance; on_rework → bootstrap.
